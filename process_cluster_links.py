...
def extract_links_from_note(note: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    patterns = {
        'github': [
            ...
            r'github\.com/[^\s\]\},]+',  # GitHub without https://
            r'github:[\s]*\[(https://github\.com/[^\s\]]+)\]',  # GitHub with brackets
            r'github:[\s]*\{(https://github\.com/[^\s\}]+)\}',  # GitHub with curly braces
            r'github:[\s]*\[(github\.com/[^\s\]]+)\]',  # GitHub with brackets, no https
        ],
        'gitlab': [
            r'https://gitlab\.com/[^\s\]\},]+',
            r'gitlab\.com/[^\s\]\},]+',  # GitLab without https://
            r'gitlab:[\s]*\[(https://gitlab\.com/[^\s\]]+)\]',
        ],
        'bitbucket': [
            r'https://bitbucket\.org/[^\s\]\},]+',
            r'bitbucket\.org/[^\s\]\},]+',  # Bitbucket without https://
            r'bitbucket:[\s]*\[(https://bitbucket\.org/[^\s\]]+)\]',
        ],
        'code_repos': [
            r'https://[^/]+\.github\.io/[^\s\]\},]+',  # GitHub Pages
            r'[^/]+\.github\.io/[^\s\]\},]+',  # GitHub Pages without https
            r'https://[^/]+\.gitlab\.io/[^\s\]\},]+',  # GitLab Pages
            r'[^/]+\.gitlab\.io/[^\s\]\},]+',  # GitLab Pages without https
        ],
        'huggingface': [
            r'https://huggingface\.co/[^\s\]\},]+',
            r'huggingface\.co/[^\s\]\},]+',  # Hugging Face without https://
    ...
    # Extract codebase links (GitHub, GitLab, Bitbucket, GitHub Pages, etc.)
    for pattern in patterns['github'] + patterns['gitlab'] + patterns['bitbucket'] + patterns['code_repos']:
    ...
    for url in all_urls:
        ...
        elif any(keyword in url.lower() for keyword in ['dataset', 'data', 'zenodo', 'figshare', 'kaggle', 'drive.google.com', 'huggingface.co/datasets']):
            if url not in data_links:
                data_links.append(url)
        # Check if it's a code-related link (other than GitHub/GitLab/Bitbucket)
        elif any(keyword in url.lower() for keyword in ['code', 'source', 'repository', 'repo', 'github.io', 'gitlab.io', 'bitbucket.io', 'retool', 'tool', 'api', 'box.com', 'drive.google.com', 'dropbox.com', 'webapp', 'cplint.eu', 'netdef', 'di.fc.ul.pt']):
    ...
    # If no codebase links found but note mentions code, add a placeholder
    if not codebase_links and note:
        code_mentions = ['code', 'codebase', 'github', 'repository', 'repo', 'source', 'implementation']
        if any(mention in note.lower() for mention in code_mentions):
            codebase_links.append("Code requested/not yet available")
    
    # Check for GitHub usernames/repos without full URLs
    if not codebase_links and note:
        github_username_pattern = r'[a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+'
        github_matches = re.findall(github_username_pattern, note)
        for match in github_matches:
            if '/' in match and not match.startswith('http'):
                codebase_links.append(f"https://github.com/{match}")
                break  # Only add the first match to avoid duplicates
    
    # If still no codebase links found, add a placeholder since codebase was required
    if not codebase_links:
        codebase_links.append("Codebase information not available in notes")
